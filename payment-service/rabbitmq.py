...
class Publisher:
    ...
    def __init__(self, logger):
        ...
        # https://github.com/pika/pika/blob/master/examples/publish.py

        #self._params = pika.connection.ConnectionParameters(
        #    host=self.HOST,
        #    virtual_host=self.VIRTUAL_HOST,
        #    credentials=pika.credentials.PlainCredentials(user, password), port=self.PORT)
        self.credentials = pika.PlainCredentials(user, password)
        self._params = pika.ConnectionParameters(host=self.HOST, credentials=self.credentials)
        self._conn = None
        self._channel = None
